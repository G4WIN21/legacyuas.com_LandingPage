...
(() => {
  ...
  function loop(){
    ...
    // Sky gradient (dim if heavy overcast)
    ...
    // Flatten contrast with cloud cover
    ...
    // ----- Clouds -----
    // Target number scales with cloud cover (30%..100% of CLOUD_MAX)
    const cloudTarget = Math.round((0.3 + 0.7*overcast) * CLOUD_MAX);
    // Slowly adjust alpha per-cloud for softness
    for (let i=0;i<CLOUD_MAX;i++){
      const c = clouds[i];
      // spawn density: only draw first N clouds
      const active = i < cloudTarget;
      const targetA = active ? (0.3 + 0.6*overcast) * (0.6 + 0.4*(1 - dayT)) : 0;
      c.a += (targetA - c.a) * 0.02;
      // move with wind
      const base = (0.2 + 0.6*(WX.windSpeed/25)) * c.z;
      c.x += base * windX;
      c.y += base * windY * 0.2; // less vertical drift
      // wrap
      if (c.x < -c.r) c.x = w + c.r;
      if (c.x > w + c.r) c.x = -c.r;
      if (c.y < h*0.05) c.y = h*0.05;
      if (c.y > h*0.7)  c.y = h*0.7;
      // draw as soft blob (3 overlapping puffs)
      if (c.a > 0.01){
        ctx.globalAlpha = c.a;
    ...
    const isSnow = (WX.snowRate > WX.rainRate && tempF <= 34) || WX.wmo===71 || WX.wmo===73 || WX.wmo===75 || WX.wmo===85 || WX.wmo===86;
    const rainIntensity = clamp((isSnow ? 0 : WX.rainRate)/8, 0, 1);   // mm/h -> 0..1
    const snowIntensity = clamp((isSnow ? WX.snowRate : 0)/5, 0, 1);   // mm/h -> 0..1
    ...
    if (raindrops.length){
      ...
      ctx.lineWidth = 1 * dpr;
      const speed = (400 + 600*rainIntensity) * dpr;
      for (const r of raindrops){
        r.x += windX * WX.windSpeed * 0.8 * dpr;
        r.y += speed * 0.016; // ~60fps step
        ctx.beginPath();
        ctx.moveTo(r.x, r.y);
    ...
    if (snowflakes.length){
      ...
      for (const s of snowflakes){
        s.sway += 0.03 + Math.random()*0.02;
        s.x += Math.cos(s.sway)*0.6*dpr + windX*WX.windSpeed*0.25*dpr;
        s.y += (40 + 60*s.speed) * 0.016 * dpr;
    ...
    // ----- Fog (from WMO fog codes or low visibility/humidity + clouds) -----
    const foggy = (WX.wmo===45 || WX.wmo===48) || (WX.visibility < 10000 && WX.humidity > 85 && WX.cloudCover > 60);
    const fogAlpha = foggy ? clamp(1 - (WX.visibility/20000), 0.1, 0.65) : 0;
    if (fogAlpha > 0.01){
    ...
    setTimeout(() => requestAnimationFrame(loop), 1000/30); // ~30 FPS
  }
